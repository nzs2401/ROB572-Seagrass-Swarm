...
class WhaleOptimization():
    ...
    """
    def __init__(self, opt_func, constraints, nsols, b, a, a_step, LonGrid, LatGrid, seagrass_coverage, depth, maximize=False):
        ...
        self.agent_paths = []
        self._LonGrid = LonGrid
        self._LatGrid = LatGrid
        self._seagrass_coverage = seagrass_coverage
        self._depth = depth
    ...
    def optimize(self):
        ...
        #Temporary list to store postions
        generation_positions = [best_sol.copy()]
        ...
        for s in ranked_sol[1:]:
            ...
            constrained_s = self._constrain_solution(new_s)                                         
            new_sols.append(constrained_s)
            generation_positions.append(constrained_s.copy())

        self._sols = np.stack(new_sols)
        self._a -= self._a_step
        self.agent_paths.append(generation_positions)   
    ...
    def _rank_solutions(self):
        """find best solution"""
        fitness = []
        # Loop over each agent (row in _sols)
        for x, y in zip(self._sols[:, 0], self._sols[:, 1]):
        # Call the user-defined objective function (planting) for a single X,Y
