...
root = os.path.join(os.path.dirname(__file__), '..') #Setup root directory (go up one level)
...
coverage_grid = np.zeros(percover.shape, dtype=int)#Create coverage classification grid
#Convert values to label
...
def planting(X, Y, LonGrid, LatGrid, seagrass_coverage, depth):
    ...
    if pc == 0.25:  return 0.25
    if pc == 0.5:   return 0.5
    if pc == 0.55:  return 0.55  
    if pc >= 0.7:   return pc
    return 0.0
...
def parse_cl_args():
    ...
    parser.add_argument("-nsols", type=int, default=1000)
    parser.add_argument("-ngens", type=int, default=1000)
...
def main():
    ...
    plot_results(env, results, algorithm_name="WOA",
                 save_path=os.path.join(os.path.dirname(__file__), "woa_results_totalcoverage.png"),
